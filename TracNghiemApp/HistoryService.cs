...
namespace TracNghiemApp
{
    class HistoryService
    {
        public List<Histories> getTop10()
        {
            ...
            conn.Open();
            string sql = "SELECT * FROM histories, users where users.id = histories.name_id  ORDER BY score DESC LIMIT 10";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                       ...
                       */
                        ...
                        double score = Convert.ToDouble(reader.GetValue(reader.GetOrdinal("score")));
